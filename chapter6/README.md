...
응용 서비스가 복잡해진다면 도메인 로직이 응용 서비스로 새고 있는지 의심해야해요. 응용 서비스는 흐름을 제어하는 역할이지, 도메인 규칙을 구현하는 곳이 아닙니다.
...
권한 검사는 보통 **표현 영역**, **응용 서비스**, **도메인 영역** 세 곳에서 수행할 수 있습니다.

### 표현 영역

인증된 사용자인지 확인하고, 인증되지 않았다면 로그인 화면으로 보내는 처리는 표현 영역에 가깝습니다. 서블릿 필터나 스프링 시큐리티 필터를 사용하면 URL별 접근 제어도 처리할 수 있습니다.
...
URL만으로 판단하기 어려운 권한은 응용 서비스 메서드 단위에서 검사합니다. 예를 들어 "관리자만 회원을 차단할 수 있다" 같은 기능 권한은 서비스 실행 전에 확인할 수 있습니다.
...
스프링 시큐리티는 AOP를 이용해 `@PreAuthorize` 같은 방식으로 서비스 메서드 권한 검사를 지원합니다.

### 도메인 영역

개별 도메인 객체 수준의 권한 검사가 필요한 경우도 있습니다. 예를 들어 "작성자만 게시글을 삭제할 수 있다"처럼 대상 객체의 상태를 봐야 하는 규칙입니다.

다만 도메인 객체 수준 권한 검사를 보안 프레임워크로 처리하려면 프레임워크 이해도가 높아야 합니다. 복잡도가 부담된다면 도메인에 맞는 권한 검사 로직을 직접 구현하는 편이 유지보수에 유리할 수 있습니다.

핵심은 권한 검사를 어디에 두든 **표현 영역의 기술이 응용/도메인 영역으로 새지 않게 하는 것**입니다.
...
- 권한 검사는 표현 영역, 응용 서비스, 도메인 영역 중 규칙의 성격에 맞는 위치에 둡니다.
